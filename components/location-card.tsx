    ...
    return (
        <div className={`flex items-center ${reverse ? 'flex-row-reverse' : 'flex-row'}`}>
            <div className={`flex-1 ${reverse ? 'text-right pr-4' : 'text-left pl-4'}`}>
                <h3 className="text-[var(--wedding-gold)] text-xl font-medium mb-1">{title}</h3>
                ...
                {/*<button className="bg-[var(--wedding-gold)] text-white text-xs px-5 py-1.5 rounded-full">ՔԱՐՏԵԶ</button>*/}
            </div>

            <div className={`w-20 h-20 relative flex-shrink-0 ${reverse ? 'mr-4' : 'ml-4'}`}>
                <div className="w-full h-full rounded-full overflow-hidden border-2 border-[var(--wedding-gold)]/20">
